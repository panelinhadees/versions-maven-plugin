...
/**
 * XML renderer for DependencyUpdatesReport creates an xml file in target
 * directory and writes report about available dependency/dependency management
 * updates.
 * 
...
public class DependencyUpdatesXmlRenderer {
	...
	/**
	 * Method wraps value in xml tag. In ex: to wrap foo in tag bar you have to pass
	 * foo as value and bar as tag. As a result you will get: <bar>foo</bar>
	 * 
	...
	/**
	 * Returns summary of dependency analysis result in xml format: current version,
	 * next available, next incremental, next minor and next major versions.
	 * 
	...
	/**
	 * Returns xml report for current dependency state with following info: current
	 * version, next available version, next incremental/minor/major if available
	 * and status ('incremental available', 'minor available', 'major available' or
	 * 'no new available')
	 * 
	...
	public static String getVersionsBlocks(ArtifactVersions versions) {
		...
		if (nextVersion != null) {
			sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(nextVersion.toString(), NEXT_VERSION))
					.append(NL);
	...
	private static String getDependencyInfoBlock(Map<Dependency, ArtifactVersions> dependencyUpdates, String blockName,
		...
		for (Entry<Dependency, ArtifactVersions> entry : dependencyUpdates.entrySet()) {
			...
			sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(dep.getArtifactId(), ARTIFACT_ID))
					.append(NL);
			sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(dep.getScope(), SCOPE)).append(NL);
			sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(dep.getClassifier(), CLASSIFIER))
					.append(NL);
	...
	private static String getVersionsInScopeBlock(ArtifactVersions av, UpdateScope scope) {
		...
		if (nextVersion != null) {
			sBuilder.append(TAB).append(TAB).append(TAB).append(OPEN_TAG).append(versionsTag).append(CLOSE_TAG)
					.append(NL);
			...
			sBuilder.append(TAB).append(TAB).append(TAB).append(OPEN_CLOSING_TAG).append(versionsTag).append(CLOSE_TAG)
					.append(NL);
		}
