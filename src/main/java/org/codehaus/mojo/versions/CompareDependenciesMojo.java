...
public class CompareDependenciesMojo extends AbstractVersionsDependencyUpdaterMojo {
	...
	/**
	 * The groupId, artifactId, and version of the remote project (POM) to which we
	 * are comparing. This should be in the form "groupId:artifactId:version"
	 */
	 ...
	 *                                                        the plugin itself
	 * @throws org.apache.maven.plugin.MojoFailureException   The plugin detected an
	 *                                                        error in the build
	...
	protected void update(ModifiedPomXMLEventReader pom)
		...
		if (getProject().getDependencyManagement() != null && isProcessingDependencyManagement()) {
			List<String> depManDiffs = compareVersions(pom, getProject().getDependencyManagement().getDependencies(),
					remoteDepsMap);
		...
		if (updatePropertyVersions) {
			Map<Property, PropertyVersions> versionProperties = this.getHelper().getVersionPropertiesMap(getProject(),
					null, null, null, true);
	...
	/**
	 * Compare the dependency versions of the current project with the dependency
	 * versions of a remote project
	 *
				...
				if (!dep.getVersion().equals(remoteVersion)) {
					...
					if (!reportMode) {
						if (PomHelper.setDependencyVersion(pom, dep.getGroupId(), dep.getArtifactId(), dep.getVersion(),
								remoteVersion, getProject().getModel())) {
	 ...
	 * <p/>
	 * The dependencies currently linked to the property must all be defined by the
	 * remote POM and they should refer to the same version. If that's the case,
	 * that same version is returned. Otherwise, <tt>null</tt> is returned
	...
	/**
	 * Creates a key that is similar to what {@link Dependency#getManagementKey()}
	 * generates for a dependency.
	 */
