...
/**
 * Finds the minimum Maven version required by a Maven project. Checks for the
 * existence of both the prerequisites.maven property and for
 * maven-enforcer-plugin:enforce goal. Returns null if no minimum version is
 * found. Checks project and it's parents recursively.
 *
 * Pros: works with Maven 3.5.0 which throws a warning if prerequisites.maven is
 * set for a non Maven-Plugin project. Cons: tightly coupled with the
 * maven-enforcer-plugin and the Xpp3Dom configuration tag.
 */
class RequiredMavenVersionFinder {
	...
	private ArtifactVersion getHighestArtifactVersion(ArtifactVersion firstMavenVersion,
			ArtifactVersion secondMavenVersion) {
	...
	private ArtifactVersion processMavenVersionRange(String versionRange) {
		...
		if (commaIndex == innerString.length() - 1) {
			...
			if (versionRange.charAt(0) == '(' && versionRange.charAt(versionRange.length() - 1) == ')') {
				// this is actually wrong - the Maven version should be higher than this, the
				// Maven version cannot be equal to this, but the Maven Enforcer plugin should
				// capture this
				return new DefaultArtifactVersion(minimumVersion);
			}
