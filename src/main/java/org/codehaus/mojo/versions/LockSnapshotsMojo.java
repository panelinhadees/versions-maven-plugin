 ...
 * version like "1.0-SNAPSHOT" could be resolved to "1.0-20090128.202731-1". If
 * a timestamped snapshot is not available, then the version will remained
...
public class LockSnapshotsMojo extends AbstractVersionsDependencyUpdaterMojo {
	...
	/**
	 * Pattern to match a timestamped snapshot version. For example
	 * 1.0-20090128.202731-1
	 */
	 ...
	 *
	 * @param artifact
	 * @return The timestamp version if exists, otherwise the original snapshot
	 *         artifact version is returned.
	 */
	 ...
	 * @param dep
	 * @return The timestamp version if exists, otherwise the original snapshot
	 *         dependency version is returned.
	 */
	private String resolveSnapshotVersion(Dependency dep) {
		...
		try {
			Artifact depArtifact = artifactFactory.createDependencyArtifact(dep.getGroupId(), dep.getArtifactId(),
					VersionRange.createFromVersionSpec(dep.getVersion()), dep.getType(), dep.getClassifier(),
					dep.getScope());
