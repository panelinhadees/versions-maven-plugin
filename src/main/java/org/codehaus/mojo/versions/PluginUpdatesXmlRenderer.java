...
/**
 * XML renderer for PluginUpdatesReport creates an xml file in target directory
 * and writes report about available plugin/plugin management updates.
 * 
...
public class PluginUpdatesXmlRenderer {
	...
	private static String getPluginsInfoBlock(Map<Plugin, PluginUpdatesDetails> pluginUpdates, String blockName,
		...
		for (Entry<Plugin, PluginUpdatesDetails> entry : pluginUpdates.entrySet()) {
			...
			sBuilder.append(TAB).append(TAB).append(TAB).append(wrapElement(plugin.getArtifactId(), ARTIFACT_ID))
					.append(NL);
